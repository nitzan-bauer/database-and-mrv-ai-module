import { excelDate, readWorkbook, type Sheet } from "./xlsx";
...
export interface ParsedSocRow {
  ...
  nPct: number | null;
  /** recomputed: TOC400 + ROC600 */
  tocPct: number | null;
  /** recomputed: TOC% x BD x thickness */
  socTPerHa: number | null;
  /** recomputed: dry mass / probe area x 100, else BD x thickness x 100 */
  soilMassTHa: number | null;
  /** what the workbook itself claimed, for comparison */
  ...
  /** set where our figure and the sheet's disagree beyond tolerance */
  warnings: string[];
}
...
export interface ParseResult {
  ...
  /** distinct values seen, for the import summary */
  summary: {
    total: number;
    soc: number;
    texture: number;
...
export function socStock(tocPct: number, bd: number, thicknessCm: number, largeCfPct = 0): number {
...
export function soilMass(
  ...
  if (dryMassG != null && probeAreaCm2) return round4((dryMassG / probeAreaCm2) * 100);
  if (bd != null && thicknessCm != null)
    return round4(bd * thicknessCm * 100 * (1 - largeCfPct / 100));
  return null;
}

export function parseSocDatasheet(buf: Buffer): ParseResult {
  const sheets = readWorkbook(buf);
  const lab: Sheet | undefined =
    sheets.find((s) => /lab\s*results/i.test(s.name)) ?? sheets[0];

  if (!lab) throw new Error("Workbook has no sheets.");
  ...
  if (!header || (header.cells[C.sampleId] ?? "").toLowerCase() !== "sample id") {
    throw new Error(
      "This does not look like CarboNature_SOC_Datasheet_v2.0 — expected 'Sample ID' in column B of row 5.",
    );
  }
  ...
  for (const row of lab.rows) {
    ...
    if (!sampleId) continue; // blank template row

    const fail = (error: string) => quarantined.push({ rowIndex: row.r, raw: c, error });
    ...
    const sampleType: "soc" | "texture" = typeRaw.startsWith("tex") ? "texture" : "soc";
    ...
    if (sampleType === "soc") {
      ...
      if (tc != null && toc400 != null && roc600 != null && tic900 != null) {
        if (Math.abs(tc - toc400 - roc600 - tic900) > 0.1) {
          fail(
            `TC ${tc} does not reconcile with TOC400 ${toc400} + ROC600 ${roc600} + TIC900 ${tic900}`,
          );
          continue;
        }
      }
    ...
    const tocPct =
      toc400 == null && roc600 == null ? null : round4((toc400 ?? 0) + (roc600 ?? 0));

    const socTPerHa =
      sampleType === "soc" && tocPct != null && bd != null && thickness != null
    ...
    if (iso === false) warnings.push("Laboratory not ISO 17025 accredited (§8.2.1.4)");
